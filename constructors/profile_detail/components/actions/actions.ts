...
export const update = action
  ...
  .action(async ({ parsedInput: values }) => {
    try {
      ...
      if (parsed.active) {
        const { result: found } = await get<Profile>({
          name: 'profile',
          paging: { limit: 1, page: 1 },
          query: { filter: 'active:true' },
        })
        if (found) {
          const [activated] = found.items
          if (activated) {
            await document.update({
              id: activated.id!,
              doc: { ...activated, active: false },
            })
          }
        }
      }
      ...
      const { result, error } = await document.update({ id: parsed.id, doc })

      if (!result && error) {
        throw new Error(error.message)
      }

      revalidatePath(`/profile/${parsed.id}`)
